...
import { collectVkInbox, replyToVkBoardComment, replyToVkPhotoComment, replyToVkPhotoTag, replyToVkUserPhotoComment, replyToVkVideoComment, replyToVkWallComment, replyToVkWallMention } from "@/social/vk/inbox";
...
export class VkAdapter extends BaseSocialAdapter {
  ...
  async replyToInbox(input: InboxReplyInput): Promise<InboxReplyResult> {
    ...
    if (input.kind !== "comment") {
      throw new UnsupportedActionError(
        "VK can reply to wall comments, photo comments, video comments, board comments, and mentions. User Direct Messages are not enabled for new apps.",
      );
    }
