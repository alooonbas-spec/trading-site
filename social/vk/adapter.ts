...
import { collectVkInbox, replyToVkBoardComment, replyToVkMarketComment, replyToVkOtherWall, replyToVkOtherWallComment, replyToVkPhotoComment, replyToVkPhotoTag, replyToVkSuggest, replyToVkUserPhotoComment, replyToVkUserVideoComment, replyToVkVideoComment, replyToVkVideoTag, replyToVkWallComment, replyToVkWallMention, replyToVkWallRepost } from "@/social/vk/inbox";
...
export class VkAdapter extends BaseSocialAdapter {
  ...
  async replyToInbox(input: InboxReplyInput): Promise<InboxReplyResult> {
    ...
    if (input.kind === "mention") {
      ...
      if (input.externalEventId.startsWith("suggest:")) {
        const sent = await replyToVkSuggest(token, {
          externalId: input.externalEventId,
          text: input.body,
        });
        return { externalMessageId: sent.externalMessageId };
    ...
    if (input.kind !== "comment") {
      throw new UnsupportedActionError(
        "VK can reply to wall comments, photo comments, video comments, board comments, market comments, and mentions. User Direct Messages are not enabled for new apps.",
      );
    }

    if (input.externalEventId.startsWith("market:")) {
      const sent = await replyToVkMarketComment(token, {
        externalId: input.externalEventId,
        text: input.body,
    ...
    if (input.externalEventId.startsWith("board:")) {
      const sent = await replyToVkBoardComment(token, {
        externalId: input.externalEventId,
        text: input.body,
        fromGroup: isVkCommunityAccount(this.context.metadata),
      });
    ...
    if (input.externalEventId.startsWith("otherwall:")) {
      const sent = await replyToVkOtherWallComment(token, {
        externalId: input.externalEventId,
        text: input.body,
      });
      return { externalMessageId: sent.externalMessageId };
