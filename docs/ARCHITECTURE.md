...
## Invite-form role picker fix (PHASE 113)

Investigating who actually calls `canAssignRole` (after PHASE 112 tested it thoroughly) turned up a real, live inconsistency: `components/settings/members-table.tsx`'s role-*change* picker already disables any role option `canAssignRole(currentRole, role)` rejects — but `components/settings/invite-member-form.tsx`'s role picker for *inviting* a new member listed all of `ASSIGNABLE_ROLES` (`ADMIN`, `OPERATOR`, `VIEWER`) unconditionally, regardless of the actor's own role. The `add_workspace_member` SQL RPC (`supabase/migrations/20260821100000_phase1_foundation.sql`) has always correctly rejected an `ADMIN` actor inviting someone as `ADMIN` ("Admins can only assign operator or viewer roles") — so an ADMIN could pick "ADMIN" in the invite form and always get a rejected request with no indication beforehand that the option wasn't actually available to them. Not a security hole (the RPC is the real boundary and was never bypassable), but a real, reproducible UX bug with a clear fix already modeled next to it in the same settings page.

`app/(dashboard)/w/[workspaceId]/settings/page.tsx` now passes `currentRole={context.role}` to `InviteMemberForm`, which disables each `SelectItem` the same way `members-table.tsx` does: `disabled={!canAssignRole(currentRole, assignableRole)}`. Real fix, not test-only — `tests/phase37-members.test.ts` gained a regression guard asserting both files wire this through (this codebase has no React component-rendering test setup, so the guard is a source-level assertion matching the file's existing style, not a rendered-DOM test).
