...
describe("PHASE 65 Graph Facebook conversations folder=pending", () => {
  ...
  it("walks the next official Pending-folder after cursor and keeps older DMs", async () => {
    ...
    expect(first.cursor).toBe(
      `creplies:done|donethreads:done|messages:2026-08-21T12:00:00+0000|otherthreads:done|pendingthreads:${encodeGraphAfter("pending-2")}|posts:done|ratingreplies:done|ratings:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
