...
describe("PHASE 69 VK community board.getComments", () => {
  ...
  it("walks the next official community board.getComments offset window and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      "board:1710000200|boardcomments:1|boardtopics:1|conversations:1|history:1|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
    ...
    expect(second.cursor).toBe(
      "board:1710000200|boardcomments:2|boardtopics:done|conversations:done|history:done|repostpages:done|wall:done|wallcomments:done|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("board.getTopics")),
    ).toHaveLength(3);
  ...
  it("skips board offsets once boardtopics:done and boardcomments:done are stored", async () => {
    ...
    expect(result.cursor).toBe(
      "board:1710000081|boardcomments:done|boardtopics:done|conversations:1|history:1|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("board.getTopics")),
    ).toHaveLength(1);
