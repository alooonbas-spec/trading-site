...
describe("PHASE 62 VK photos.getComments on tagged photos", () => {
  ...
  it("walks the next official getComments offset window and keeps older tagged-photo comments", async () => {
    ...
    expect(first.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|phototags:1710000100|repostpages:1|userphoto:1710000200|userphotocomments:2|userphotos:done|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
  ...
  it("collects comments on tagged photos that have no caption text", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphoto:1710000200|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
  });
  ...
  it("skips getComments offset once userphotocomments:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|phototags:1710000100|repostpages:1|userphoto:1710000081|userphotocomments:done|userphotos:done|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getComments")),
    ).toHaveLength(1);
