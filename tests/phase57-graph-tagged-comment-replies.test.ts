...
describe("PHASE 57 Graph nested replies on tagged comments", () => {
  ...
  it("walks the next official Facebook tagged-comment replies after cursor and keeps older nested replies", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|creplies:${encodeGraphReplies({ tagc1: "tag-reply-2" })}|donethreads:done|mentions:2026-08-21T12:00:00+0000|otherthreads:done|pendingthreads:done|posts:done|ratingreplies:done|ratings:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
