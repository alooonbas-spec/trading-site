...
describe("PHASE 57 Graph nested replies on tagged comments", () => {
  ...
  it("seeds Facebook creplies from extra tagged comment pages", async () => {
    const fetchMock = vi.fn(async (url: string) => {
      const target = String(url);
      if (target.startsWith(`${FACEBOOK_GRAPH_ORIGIN}/me/accounts`)) {
        return new Response(
            ...
            data: [
              {
                ...
                comments: {
                  data: [
                    {
                      id: "seedr",
                      message: "nested on extra tagged comment",
                      from: { id: "801", name: "Replier" },
                      created_time: "2026-08-20T09:00:00+0000",
                    },
                  ],
                  paging: { cursors: { after: "seed-reply-2" } },
                },
              },
            ],
          }),
    ...
    expect(second.messages.map((item) => item.externalId)).toEqual(["seedc", "seedr"]);
    expect(second.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|creplies:${encodeGraphReplies({ seedc: "seed-reply-2" })}|mentions:2026-08-21T12:00:00+0000|posts:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
