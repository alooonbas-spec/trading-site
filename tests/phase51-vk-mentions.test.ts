...
describe("PHASE 51 VK newsfeed.getMentions", () => {
  ...
  it("walks the next official getMentions offset window and keeps older mentions", async () => {
    ...
    expect(first.cursor).toBe("mentionpages:1|mentions:1710000200|otherwall:1|otherwallcomments:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
    expect(
  ...
  it("skips getMentions offset once mentionpages:done is stored", async () => {
    ...
    expect(result.cursor).toBe("mentionpages:done|mentions:1710000081|otherwall:1|otherwallcomments:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("newsfeed.getMentions")),
    ).toHaveLength(1);
  });
