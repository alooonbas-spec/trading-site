...
describe("PHASE 46 X mention and DM pagination_token", () => {
  ...
  it("walks the next official dm_events pagination_token and keeps older DMs", async () => {
    ...
    expect(first.cursor).toBe(`dmpages:${encodeXPageToken("page-2")}|dms:80|mentionpages:done|quotepages:done|replypages:done|retweetpages:done|tweetpages:done`);
    expect(
    ...
    expect(second.cursor).toBe("dmpages:done|dms:80|mentionpages:done|quotepages:done|replypages:done|retweetpages:done|tweetpages:done");
    expect(
  ...
  it("skips mention pagination_token once mentionpages:done is stored", async () => {
    ...
    expect(result.cursor).toBe("dmpages:done|dms:70|mentionpages:done|mentions:60|quotepages:done|replypages:done|retweetpages:done|tweetpages:done");
  });
});
