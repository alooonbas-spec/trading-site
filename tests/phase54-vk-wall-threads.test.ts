...
describe("PHASE 54 VK wall comment threads", () => {
  ...
  it("walks the next official comment_id thread window and keeps older nested replies", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:1710000200|mentionpages:1|photocomments:1|videocomments:1|videos:1|wall:1|wallcomments:1|wallthreads:${encodeVkThreadMap({ "10_20_80": "1" })}`,
    );
    ...
    expect(second.cursor).toBe(
      `comments:1710000200|mentionpages:done|photocomments:done|videocomments:done|videos:done|wall:done|wallcomments:done|wallthreads:${encodeVkThreadMap({ "10_20_80": "2" })}`,
    );
  });
