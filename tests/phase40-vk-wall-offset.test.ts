...
describe("PHASE 40 VK wall.get offset", () => {
  ...
  it("walks the next official wall.get offset window after wall:1", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000200|mentionpages:1|photocomments:1|videocomments:1|videos:1|videothreads:done|wall:2|wallcomments:1|wallthreads:done");
  ...
  it("stores wall:done when the older wall.get window is short", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000200|mentionpages:1|photocomments:1|videocomments:1|videos:1|videothreads:done|wall:done|wallcomments:1|wallthreads:done");
  });
  ...
  it("skips wall.get offset after wall:done and still filters latest comments by unix watermark", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000099|mentionpages:1|photocomments:1|videocomments:1|videos:1|videothreads:done|wall:done|wallcomments:1|wallthreads:done");
