...
describe("PHASE 79 VK video.getUserVideos", () => {
  ...
  it("walks the next official getUserVideos offset window and keeps older tagged videos", async () => {
    ...
    expect(first.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videotags:1710000200|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
  ...
  it("skips getUserVideos offset once uservideos:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:done|uservideothreads:done|videocomments:1|videos:1|videotags:1710000081|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("video.getUserVideos")),
    ).toHaveLength(1);
