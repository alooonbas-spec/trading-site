...
describe("PHASE 60 Graph nested replies on Facebook rating-story comments", () => {
  ...
  it("walks the next official Facebook rating-comment replies after cursor and keeps older nested replies", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|creplies:${encodeGraphReplies({ ratec1: "rate-reply-2" })}|posts:done|ratingreplies:done|ratings:done|replies:done|reviews:2026-08-21T12:00:00+0000|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
    ...
    expect(second.cursor).toBe(
      "comments:2026-08-21T12:00:00+0000|creplies:done|posts:done|ratingreplies:done|ratings:done|replies:done|reviews:2026-08-21T12:00:00+0000|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
  ...
  it("skips Facebook rating-comment replies after paging once creplies:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "comments:2026-08-21T10:00:00+0000|creplies:done|posts:done|ratingreplies:done|ratings:done|replies:done|reviews:2026-08-21T09:00:00+0000|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
    expect(fetchMock.mock.calls.filter(([url]) => String(url).includes("/ratec1/comments"))).toHaveLength(0);
  });
});
