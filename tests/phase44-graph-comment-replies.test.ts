...
describe("PHASE 44 Graph nested comment after paging", () => {
  ...
  it("walks the next official Facebook comment after cursor and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|creplies:done|donethreads:done|otherthreads:done|posts:done|ratingreplies:done|ratings:done|replies:${encodeGraphReplies({ "555_1": "cmt-2" })}|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
