...
describe("PHASE 44 Graph nested comment after paging", () => {
  ...
  it("walks the next official Facebook comment after cursor and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|posts:done|replies:${encodeGraphReplies({ "555_1": "cmt-2" })}|threadmsgs:done|threads:done`,
    );
    ...
    expect(second.cursor).toBe(
      "comments:2026-08-21T12:00:00+0000|posts:done|replies:done|threadmsgs:done|threads:done",
    );
  ...
  it("skips Instagram comment after paging once replies:done is stored", async () => {
    ...
    const result = await new InstagramAdapter({
      ...
      workspaceId: "w",
      socialAccountId: "a",
      cursor: "comments:2026-08-21T08:00:00+0000|posts:done|replies:done|threadmsgs:done|threads:done",
    });
    expect(result.messages.map((item) => item.externalId)).toEqual(["new-c"]);
    expect(result.cursor).toBe(
      "comments:2026-08-21T10:00:00+0000|posts:done|replies:done|threadmsgs:done|threads:done",
    );
