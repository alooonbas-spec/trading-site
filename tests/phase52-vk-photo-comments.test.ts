...
describe("PHASE 52 VK photos.getAllComments", () => {
  ...
  it("walks the next official getAllComments offset window and keeps older photo comments", async () => {
    ...
    expect(first.cursor).toBe("mentionpages:1|otherwall:1|photocomments:1|photos:1710000200|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
    expect(
  ...
  it("skips getAllComments offset once photocomments:done is stored", async () => {
    ...
    expect(result.cursor).toBe("mentionpages:1|otherwall:1|photocomments:done|photos:1710000081|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getAllComments")),
    ).toHaveLength(1);
  });
