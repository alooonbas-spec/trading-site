...
describe("PHASE 52 VK photos.getAllComments", () => {
  ...
  it("walks the next official getAllComments offset window and keeps older photo comments", async () => {
    ...
    expect(first.cursor).toBe("mentionpages:1|photocomments:1|photos:1710000200|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
    expect(
    ...
    expect(second.cursor).toBe("mentionpages:done|photocomments:2|photos:1710000200|repostpages:done|userphotocomments:done|userphotos:done|uservideocomments:done|uservideos:done|uservideothreads:done|videocomments:done|videos:done|videothreads:done|wall:done|wallcomments:done|wallthreads:done");
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getAllComments")),
    ).toHaveLength(3);
  });
  ...
  it("skips getAllComments offset once photocomments:done is stored", async () => {
    ...
    expect(result.cursor).toBe("mentionpages:1|photocomments:done|photos:1710000081|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getAllComments")),
    ).toHaveLength(1);
  });
  ...
  it("isolates unavailable photos.getAllComments for community inbox", async () => {
    const fetchMock = vi.fn(async (url: string) => {
      const target = String(url);
      if (target === vkMethodUrl("photos.getUserPhotos") || target === vkMethodUrl("video.getUserVideos") || target === vkMethodUrl("photos.getComments")) {
