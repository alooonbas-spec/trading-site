...
describe("PHASE 63 Graph Facebook conversations folder=other", () => {
  ...
  it("walks the next official Other-folder after cursor and keeps older DMs", async () => {
    ...
    expect(first.cursor).toBe(
      `creplies:done|messages:2026-08-21T12:00:00+0000|otherthreads:${encodeGraphAfter("other-2")}|posts:done|ratingreplies:done|ratings:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
    ...
    expect(second.cursor).toBe(
      "creplies:done|messages:2026-08-21T12:00:00+0000|otherthreads:done|posts:done|ratingreplies:done|ratings:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
    expect(
  ...
  it("skips Other-folder after paging once otherthreads:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "creplies:done|messages:2026-08-21T09:00:00+0000|otherthreads:done|posts:done|ratingreplies:done|ratings:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
    expect(
      fetchMock.mock.calls.filter(
        ([url]) => String(url).includes("/555/conversations") && String(url).includes("folder=other"),
