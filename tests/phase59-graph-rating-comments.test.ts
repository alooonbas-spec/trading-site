...
describe("PHASE 59 Graph comments on Facebook rating stories", () => {
  ...
  it("walks the next official Facebook rating-story comment after cursor and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|creplies:done|posts:done|ratings:done|ratingreplies:${encodeGraphReplies({ st9001: "rate-cmt-2" })}|replies:done|reviews:2026-08-21T12:00:00+0000|tagged:done|taggedreplies:done|threadmsgs:done|threads:done`,
    );
    ...
    expect(second.cursor).toBe(
      "comments:2026-08-21T12:00:00+0000|creplies:done|posts:done|ratings:done|ratingreplies:done|replies:done|reviews:2026-08-21T12:00:00+0000|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
  ...
  it("skips Facebook rating-story comments after paging once ratingreplies:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "comments:2026-08-21T10:00:00+0000|creplies:done|posts:done|ratings:done|ratingreplies:done|replies:done|reviews:2026-08-21T09:00:00+0000|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
    expect(fetchMock.mock.calls.filter(([url]) => String(url).includes("/st9001/comments"))).toHaveLength(0);
  });
  ...
  it("collects comments on Facebook ratings that have a story id even without review text", async () => {
    ...
    expect(result.cursor).toBe(
      "comments:2026-08-21T12:00:00+0000|creplies:done|posts:done|ratings:done|ratingreplies:done|replies:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done",
    );
  });
