...
describe("PHASE 70 VK community market.getComments", () => {
  ...
  it("walks the next official community market.getComments offset window and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      "conversations:1|history:1|market:1710000200|marketcomments:1|marketitems:1|otherwall:1|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
    ...
    expect(second.cursor).toBe(
      "conversations:done|history:done|market:1710000200|marketcomments:2|marketitems:done|otherwall:done|repostpages:done|wall:done|wallcomments:done|wallthreads:done",
    );
  ...
  it("skips market offsets once marketitems:done and marketcomments:done are stored", async () => {
    ...
    expect(result.cursor).toBe(
      "conversations:1|history:1|market:1710000081|marketcomments:done|marketitems:done|otherwall:1|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("market.get"))).toHaveLength(1);
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("market.getComments")),
