...
describe("PHASE 43 Graph feed and media after paging", () => {
  ...
  it("walks the next official Facebook feed after cursor and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      `comments:2026-08-21T12:00:00+0000|posts:${encodeGraphAfter("feed-2")}|threads:done`,
    );
    ...
    expect(second.cursor).toBe("comments:2026-08-21T12:00:00+0000|posts:done|threads:done");
  ...
  it("skips Instagram media after paging once posts:done is stored", async () => {
    ...
    expect(result.cursor).toBe("comments:2026-08-21T10:00:00+0000|posts:done|threads:done");
