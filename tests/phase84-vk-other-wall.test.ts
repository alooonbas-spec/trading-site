...
describe("PHASE 84 VK wall.get filter=others", () => {
  ...
  it("walks the next official wall.get others offset window and keeps older visitor posts", async () => {
    ...
    expect(first.cursor).toBe(
      "mentionpages:1|others:1710000200|otherwall:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
    ...
    expect(second.cursor).toBe(
      "mentionpages:done|others:1710000200|otherwall:2|photocomments:done|repostpages:done|userphotocomments:done|userphotos:done|uservideocomments:done|uservideos:done|uservideothreads:done|videocomments:done|videos:done|videothreads:done|wall:done|wallcomments:done|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(
        ([, init]) => String(init?.body ?? "").includes("filter=others") && String(init?.body ?? "").includes("offset=10"),
  ...
  it("skips others offset once otherwall:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|others:1710000081|otherwall:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
  ...
  it("collects community wall.get others with owner_id of the community wall", async () => {
    ...
    expect(result.cursor).toBe(
      "conversations:1|history:1|others:1710000200|otherwall:1|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
  });
