...
describe("PHASE 77 VK wall.getReposts", () => {
  ...
  it("walks the next official getReposts offset window and keeps older reposts", async () => {
    ...
    expect(first.cursor).toBe(
      "mentionpages:1|photocomments:1|repostpages:1|reposts:1710000200|userphotocomments:1|userphotos:1|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    ...
    expect(second.cursor).toBe(
      "mentionpages:done|photocomments:done|repostpages:2|reposts:1710000200|userphotocomments:done|userphotos:done|videocomments:done|videos:done|videothreads:done|wall:done|wallcomments:done|wallthreads:done",
    );
  ...
  it("skips getReposts offset once repostpages:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|photocomments:1|repostpages:done|reposts:1710000081|userphotocomments:1|userphotos:1|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("wall.getReposts"))).toHaveLength(1);
  });
