...
describe("PHASE 81 VK tagged video comment threads", () => {
  ...
  it("walks the next official tagged-video comment_id thread window and keeps older nested replies", async () => {
    ...
    expect(first.cursor).toBe(
      `mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideo:1710000200|uservideocomments:1|uservideos:1|uservideothreads:${encodeVkThreadMap({ "77_20_80": "1" })}|videocomments:1|videos:1|videotags:1710000100|videothreads:done|wall:1|wallcomments:1|wallthreads:done`,
    );
