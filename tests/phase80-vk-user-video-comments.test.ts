...
describe("PHASE 80 VK video.getComments on tagged videos", () => {
  ...
  it("walks the next official getComments offset window and keeps older tagged-video comments", async () => {
    ...
    expect(first.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideo:1710000200|uservideocomments:2|uservideos:done|uservideothreads:done|videocomments:1|videos:1|videotags:1710000100|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
  ...
  it("collects comments on tagged videos that have no caption text", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideo:1710000200|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
  });
  ...
  it("skips getComments offset once uservideocomments:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|otherwall:1|otherwallcomments:1|otherwallthreads:done|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideo:1710000081|uservideocomments:done|uservideos:done|uservideothreads:done|videocomments:1|videos:1|videotags:1710000100|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("video.getComments")),
    ).toHaveLength(1);
