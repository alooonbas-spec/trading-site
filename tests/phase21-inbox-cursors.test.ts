...
describe("PHASE 21 collector watermarks", () => {
  ...
  it("sends X mention since_id from the mentions watermark and returns a split cursor", async () => {
    ...
    expect(result.cursor).toBe("dmpages:done|dms:80|mentionpages:done|mentions:60|quotepages:done|replypages:done|replytopages:done|retweetpages:done|tweetpages:done");
  ...
  it("advances a VK unix comment cursor and skips older wall comments", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000099|mentionpages:1|otherwall:1|otherwallcomments:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done");
  });
});
