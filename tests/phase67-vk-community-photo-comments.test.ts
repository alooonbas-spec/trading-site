...
describe("PHASE 67 VK community photos.getAllComments", () => {
  ...
  it("walks the next official community getAllComments offset window and keeps older photo comments", async () => {
    ...
    expect(first.cursor).toBe(
      "conversations:1|history:1|photocomments:1|photos:1710000200|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
    ...
    expect(second.cursor).toBe(
      "conversations:done|history:done|photocomments:2|photos:1710000200|repostpages:done|wall:done|wallcomments:done|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getAllComments")),
    ).toHaveLength(3);
  ...
  it("skips getAllComments offset once photocomments:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "conversations:1|history:1|photocomments:done|photos:1710000200|repostpages:1|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getAllComments")),
    ).toHaveLength(1);
