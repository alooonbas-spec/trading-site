...
describe("PHASE 41 VK wall.getComments offset", () => {
  ...
  it("walks the next official getComments offset window after wallcomments:1", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000200|mentionpages:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideos:1|videocomments:1|videos:1|videothreads:done|wall:done|wallcomments:2|wallthreads:done");
    expect(
  ...
  it("stores wallcomments:done when the older getComments window is short", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000200|mentionpages:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideos:1|videocomments:1|videos:1|videothreads:done|wall:done|wallcomments:done|wallthreads:done");
  });
  ...
  it("skips getComments offset after wallcomments:done", async () => {
    ...
    expect(result.cursor).toBe("comments:1710000081|mentionpages:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideos:1|videocomments:1|videos:1|videothreads:done|wall:done|wallcomments:done|wallthreads:done");
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("wall.getComments")),
    ).toHaveLength(1);
  });
