...
describe("PHASE 61 VK photos.getUserPhotos", () => {
  ...
  it("walks the next official getUserPhotos offset window and keeps older tagged photos", async () => {
    ...
    expect(first.cursor).toBe(
      "mentionpages:1|photocomments:1|phototags:1710000200|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
    ...
    expect(second.cursor).toBe(
      "mentionpages:done|photocomments:done|phototags:1710000200|repostpages:done|userphotocomments:done|userphotos:2|uservideocomments:done|uservideos:done|videocomments:done|videos:done|videothreads:done|wall:done|wallcomments:done|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getUserPhotos")),
    ).toHaveLength(3);
  ...
  it("skips getUserPhotos offset once userphotos:done is stored", async () => {
    ...
    expect(result.cursor).toBe(
      "mentionpages:1|photocomments:1|phototags:1710000081|repostpages:1|userphotocomments:1|userphotos:done|uservideocomments:1|uservideos:1|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("photos.getUserPhotos")),
    ).toHaveLength(1);
