...
describe("PHASE 50 Graph tagged mention after paging", () => {
  ...
  it("walks the next official Facebook tagged after cursor and keeps older mentions", async () => {
    const fetchMock = vi.fn(async (url: string) => {
      ...
      if (target.includes("/555/feed") || target.includes("/555/conversations") || target.includes("/555/videos")) {
        return new Response(JSON.stringify({ data: [] }), { status: 200 });
      }
      if (target.includes("/555/ratings") || target.includes("/555/videos")) {
        return new Response(JSON.stringify({ data: [] }), { status: 200 });
      }
    ...
    expect(first.cursor).toBe(
      `creplies:done|donethreads:done|mentions:2026-08-21T12:00:00+0000|otherthreads:done|pendingthreads:done|posts:done|ratingreplies:done|ratings:done|replies:done|spamthreads:done|tagged:${encodeGraphAfter("tag-2")}|taggedreplies:done|threadmsgs:done|threads:done|videoreplies:done|videos:done`,
    );
    ...
    expect(second.cursor).toBe(
      "creplies:done|donethreads:done|mentions:2026-08-21T12:00:00+0000|otherthreads:done|pendingthreads:done|posts:done|ratingreplies:done|ratings:done|replies:done|spamthreads:done|tagged:done|taggedreplies:done|threadmsgs:done|threads:done|videoreplies:done|videos:done",
    );
