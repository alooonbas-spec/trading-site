...
describe("PHASE 68 VK community video.getComments", () => {
  ...
  it("walks the next official community video.getComments offset window and keeps older comments", async () => {
    ...
    expect(first.cursor).toBe(
      "conversations:1|history:1|otherwall:1|otherwallcomments:1|repostpages:1|video:1710000200|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    ...
    expect(second.cursor).toBe(
      "conversations:done|history:done|otherwall:done|otherwallcomments:done|repostpages:done|video:1710000200|videocomments:2|videos:done|videothreads:done|wall:done|wallcomments:done|wallthreads:done",
    );
  ...
  it("skips video offsets once videos:done and videocomments:done are stored", async () => {
    ...
    expect(result.cursor).toBe(
      "conversations:1|history:1|otherwall:1|otherwallcomments:1|repostpages:1|video:1710000081|videocomments:done|videos:done|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
    expect(fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("video.get"))).toHaveLength(1);
    expect(
      fetchMock.mock.calls.filter(([url]) => String(url) === vkMethodUrl("video.getComments")),
