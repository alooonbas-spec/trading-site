...
describe("PHASE 55 VK video comment threads", () => {
  ...
  it("walks the next official video comment_id thread window and keeps older nested replies", async () => {
    ...
    expect(first.cursor).toBe(
      `mentionpages:1|otherwall:1|otherwallcomments:1|photocomments:1|repostpages:1|userphotocomments:1|userphotos:1|uservideocomments:1|uservideos:1|uservideothreads:done|video:1710000200|videocomments:1|videos:1|videothreads:${encodeVkThreadMap({ "10_20_80": "1" })}|wall:1|wallcomments:1|wallthreads:done`,
    );
  ...
  it("isolates unavailable video.getComments for community inbox", async () => {
    ...
    expect(result.cursor).toBe(
      "conversations:1|history:1|otherwall:1|otherwallcomments:1|repostpages:1|videocomments:1|videos:1|videothreads:done|wall:1|wallcomments:1|wallthreads:done",
    );
